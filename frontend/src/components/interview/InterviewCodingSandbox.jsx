...
const InterviewCodingSandbox = ({ problemId, interviewId, onSubmitAnswer, disabled }) => {
  ...
  // Compact, fixed-viewport layout (§ coding sandbox resize): two columns on wide screens
  // (question+reference on the left in its own scroll region, editor+console on the right),
  // collapsing to one stacked column on narrow screens. The OUTER page never scrolls — only
  // these two internal regions do — which is what makes the whole exercise fit in one view
  // instead of forcing the candidate to scroll a tall page under a ticking 4-minute timer.
  return (
    <div className="w-full h-[min(78vh,760px)] min-h-[440px] text-left">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 h-full">
        {/* Left column — question + reference material, one scroll region for both. */}
        <div className="flex flex-col gap-3 h-full min-h-0 overflow-y-auto pr-1">
          <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm p-3 md:p-4 space-y-2.5">
            ...
            {/* The scenario, not the problem title, is the actual question — so it is rendered
                as a highlighted block immediately under the heading, on its own. */}
            <div className="rounded-lg border-l-4 border-primary-500 bg-primary-50/60 dark:bg-primary-500/5 border-y border-r border-slate-200 dark:border-slate-800 p-3">
              <p className="text-[9px] font-bold uppercase tracking-widest text-primary-600 dark:text-primary-400 mb-1.5">
                The Scenario
              </p>
          ...
          {/* Reference material (schema, sample data, examples, constraints) — flows in the
              same scroll region as the question above rather than nesting its own scrollbar. */}
          {hasDetails && (
            <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm p-3 md:p-4 space-y-3">
        ...
        {/* Right column — editor + console, height-capped to the same viewport budget as the
            left column so the pair never grows the page. */}
